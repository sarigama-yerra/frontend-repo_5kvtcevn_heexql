...
export default function Contact() {
  const [form, setForm] = useState({ nombre: '', email: '', telefono: '', tipo: '', fecha: '', mensaje: '' })
  ...
  return (
    <section id="contacto" className="bg-gradient-to-b from-white to-emerald-50/60">
      <div className="max-w-7xl mx-auto px-6 py-16 grid lg:grid-cols-2 gap-10 items-start">
        <div>
          <h2 className="text-2xl font-semibold text-emerald-800">Cuéntanos sobre tu evento</h2>
          <p className="mt-2 text-slate-600">Completa el formulario y te contactaremos para enviarte una cotización y agendar una visita al local.</p>
          <div className="mt-6 p-4 rounded-xl border border-emerald-100 bg-white">
            ...
            <div className="mt-1 text-slate-900">contacto@solaz-eventos.com · +56 9 5555 5555</div>
        ...
        <form onSubmit={onSubmit} className="rounded-xl border border-emerald-100 bg-white p-6 shadow-sm">
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm text-slate-700 mb-1">Tipo de evento</label>
              <select name="tipo" value={form.tipo} onChange={onChange} className="w-full px-3 py-2 rounded-md border border-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-400">
          ...
          <div className="mt-4 grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-slate-700 mb-1">Fecha aproximada</label>
              <input type="date" name="fecha" value={form.fecha} onChange={onChange} className="w-full px-3 py-2 rounded-md border border-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-400" />
            </div>
            <div>
          ...
          <button type="submit" className="mt-6 inline-flex items-center px-5 py-3 rounded-md bg-amber-500 text-white font-medium shadow-sm hover:shadow-md hover:bg-emerald-600 transition-all">
            {sent ? 'Solicitud enviada ✔' : 'Enviar solicitud'}
          </button>
          {sent && <div className="mt-3 text-sm text-emerald-700">Gracias. Te contactaremos a la brevedad para coordinar tu visita.</div>}
        </form>
      </div>
    </section>
  )
